...
% Instead of forcing white patches to 1, we force them toward the mean white value.
% mean_white = mean(white_rgb_values, 1);  % [mean_R, mean_G, mean_B]
mean_white = [1,1,1];  % [mean_R, mean_G, mean_B]
% [~, brightest_idx] = max(sum(white_rgb_values, 2));  % Find index of brightest patch
% mean_white = white_rgb_values(brightest_idx, :);  % Extract its RGB
...
% Apply Gaussian smoothing to the gain maps to reduce center concentration
sigma = 15; % You can adjust the smoothing strength (higher sigma = more smoothing)

% gain_r_map = imgaussfilt(gain_r_map, sigma);
% gain_g_map = imgaussfilt(gain_g_map, sigma);
% gain_b_map = imgaussfilt(gain_b_map, sigma);
%% Clamp the gain maps to avoid extreme corrections (adjust)
gain_min = 0.1; gain_max = 1.9;
...
% figure; imagesc(gain_r_map); colorbar; title('Red Gain Map');
% figure; imagesc(gain_g_map); colorbar; title('Green Gain Map');
% figure; imagesc(gain_b_map); colorbar; title('Blue Gain Map');
...
title('Relative White Balanced Image');
