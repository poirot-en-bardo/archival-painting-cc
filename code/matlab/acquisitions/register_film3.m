...
movCube = flip(S.wb_cube, 1); % Flip vertically as per your pipeline
wl_mov  = S.wl;
...
movCrop = movCube( ...
    ...
    pos(1):(pos(1)+pos(3)-1), ...
    : );
fixCrop = fixCube; % Use full fixed cube, or crop similarly if desired
...
% --- SURF feature-based registration (with your parameters) ---
gray_fix = rgb2gray(RGB_fix);
gray_mov = rgb2gray(RGB_mov);

% Detect SURF features
points_fix = detectSURFFeatures(gray_fix);
points_mov = detectSURFFeatures(gray_mov);

% Extract feature descriptors
[features_fix, validPts_fix] = extractFeatures(gray_fix, points_fix);
[features_mov, validPts_mov] = extractFeatures(gray_mov, points_mov);

% Match features with custom parameters
indexPairs = matchFeatures(features_fix, features_mov, ...
    'MaxRatio', 0.8, ...
    'MatchThreshold', 100, ...
    'Unique', true);

matched_fix = validPts_fix(indexPairs(:,1));
matched_mov = validPts_mov(indexPairs(:,2));

% Estimate transformation (projective, your parameters)
if numel(matched_fix) >= 4
    [tformObj, inlierIdx] = estimateGeometricTransform2D( ...
        matched_mov, matched_fix, 'projective', ...
        'MaxDistance', 50, 'Confidence', 95, 'MaxNumTrials', 10000);
else
    error('Not enough matched features for registration.');
end
...
figure; imshowpair(RGB_fix, reg_mov_rgb, 'blend');
title('Overlay AFTER SURF Registration (blend)','FontSize',14);

figure; imshowpair(RGB_fix, reg_mov_rgb);
title('D50 RGBs AFTER SURF Registration (Left: Fixed HSI, Right: Reg. MSI)','FontSize',14);
