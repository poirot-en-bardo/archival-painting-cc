...
for i = 1:length(folders)
    ...
    if ~isempty(hdr_file) && ~isempty(img_file)
        ...
        xa = max(1, round(r(1))); ya = max(1, round(r(2)));
        xb = min(size(cube_mov,2), xa + max(1, round(r(3))) - 1);
        yb = min(size(cube_mov,1), ya + max(1, round(r(4))) - 1);
        ...
        mov_XYZ = ref2xyz(ill_interp(:), cmf_interp, reshape(cube_mov, [], size(cube_mov,3)));
        ...
        cube_mov = imresize(cube_mov, scale);
        cube_mov = flip(cube_mov, 2);  % Flip horizontally
        cube_mov = flip(cube_mov, 1);  % Flip vertically for TIFF
...
figure; imagesc(slice); axis image; colormap gray; title('Crop TIFF image');
        ...
        % Use LED-based spectral estimation
        ...
        mov_XYZ = cube_flat * cmf_band;

        k_factor = sum(d50_band .* cmf_band(:,2));
        mov_XYZ = mov_XYZ / k_factor;
    end
