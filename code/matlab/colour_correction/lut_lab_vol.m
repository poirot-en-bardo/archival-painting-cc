...
%% ---------------- Train-Test Split with Filtering ----------------
...
% --- Compute the training set volume using an alphaShape ---
% Use a very large alpha value (Inf) to create the convex hull of the training data.
shp = alphaShape(lab_input_train(:,1), lab_input_train(:,2), lab_input_train(:,3), Inf);

% Filter the initial test indices to only keep points inside the training volume.
insideLogical = inShape(shp, lab_input_flat(initial_test_idx,1), ...
                              lab_input_flat(initial_test_idx,2), ...
                              lab_input_flat(initial_test_idx,3));
filtered_test_idx = initial_test_idx(insideLogical);

% to maintain a similar test count, randomly sample
desiredTestCount = numel(initial_test_idx);
if numel(filtered_test_idx) > desiredTestCount
    filtered_test_idx = filtered_test_idx(randperm(numel(filtered_test_idx), desiredTestCount));
end
test_idx = filtered_test_idx;

%% ---------------- Build 3D LUT in Lab Space using Training Data ----------------
% Create scattered interpolants mapping input Lab to reference Lab.
...
%% ---------------- Convert Corrected Lab to RGB for Display ----------------
% Reshape the corrected Lab back to a 10x14 patch for display.
lab_corrected_2d = reshape(lab_corrected_flat, 10, 14, 3);
rgb_corrected = lab2rgb(lab_corrected_2d, 'ColorSpace', 'prophoto-rgb');
...
%% ---------------- Visualize Training Volume and Test Points ----------------
figure('Name','Training vs Test Points with Volume in Lab Space');
% Plot training points in blue
scatter3(lab_input_train(:,1), lab_input_train(:,2), lab_input_train(:,3), 80, 'b', 'filled');
hold on;
% Plot test points (filtered to be inside the training volume) in red
scatter3(lab_input_flat(test_idx,1), lab_input_flat(test_idx,2), lab_input_flat(test_idx,3), 80, 'r', 'filled');
% Plot the convex hull of the training set
...
title('Training vs Test Points with Training Volume in Lab Space');
grid on;
view(3);
hold off;
