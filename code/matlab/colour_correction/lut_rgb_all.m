...
%% ---------------- Use All Data for LUT Training ----------------
% Flatten the RGB data (each row is one patch)
rgb_input_flat = reshape(rgb_input_2d, [], 3);
rgb_ref_flat   = reshape(rgb_ref_2d, [], 3);

% Use all data points for training
rgb_input_train = rgb_input_flat;
rgb_ref_train   = rgb_ref_flat;

%% ---------------- Build 3D LUT in RGB Space using All Data ----------------
% Create a scattered interpolant for each RGB channel mapping input -> reference.
...
%% ---------------- Evaluate Correction ----------------
% Convert both corrected and reference RGB to Lab for evaluation.
...
test_idx = 1:size(rgb_ref_flat,1);
...
% range of RGB input values
min_val = min(rgb_input_train);
max_val = max(rgb_input_train);
numSamples = 17;  %grid sampling res
...
%% Visualising the input and the corrected values 
