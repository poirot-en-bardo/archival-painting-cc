...
film_path = '/home/oem/eliza/data/xyz_lab_rgb/film/yoda_halogen_fuji_exp0_balanced_registered_xyz.mat';
cc_film_path = '/home/oem/eliza/data/xyz_lab_rgb/colorchecker/yoda_halogen_fuji_exp0_balanced_colorchecker.mat';
...
base_output_dir = '/home/oem/eliza/masters-thesis/results/plots/results/cc';
...
%%
cc_rgb_ref = cc_ref.RGB_ref;
cc_rgb_film = cc_film.patchRGB;
...
film_rgb_srgb          = xyz2rgb(film_data.XYZ_img ./100, 'ColorSpace', 'srgb');
corrected_rgb_srgb     = xyz2rgb(prophoto2xyz(corrected_rgb, true) ./100, 'ColorSpace', 'srgb');
painting_before_srgb   = xyz2rgb(painting_before.XYZ_img ./100, 'ColorSpace', 'srgb');
painting_after_srgb    = xyz2rgb(painting_after.XYZ_img ./100, 'ColorSpace', 'srgb');
...
%% Create and save figure: HSI before, after, and ΔE between

fig_hsi_deltaE = figure('Units', 'normalized', 'Position', [0.1, 0.2, 1.1, 0.9]);
tiledlayout(1, 3, 'Padding', 'compact', 'TileSpacing', 'compact');
...
% 1. Before ageing (sRGB)
nexttile;
imshow(painting_before_srgb_img);
title('HSI Painting Before Ageing', 'FontSize', fontSize, 'FontWeight', 'bold');

% 2. After ageing (sRGB)
nexttile;
imshow(painting_after_srgb_img);
title('HSI Painting After Ageing', 'FontSize', fontSize, 'FontWeight', 'bold');

% 3. ΔE map (before vs after)
nexttile;
imagesc(deltaE_painting_img);
axis image off;
title('ΔE_{00} Map: Before vs. After', 'FontSize', fontSize, 'FontWeight', 'bold');
c = colorbar;
clim(clim_range);
c.Label.String = 'ΔE_{00}';
c.Label.FontSize = labelSize;
...
% Save the figure
deltaE_painting_vis_path = fullfile(outputDir, [film_base_name '_painting_before_after_visual.png']);
saveas(fig_hsi_deltaE, deltaE_painting_vis_path);
fprintf('Saved HSI before/after/ΔE figure to: %s\n', deltaE_painting_vis_path);
...
%%
output_path = '/home/oem/eliza/mac-shared/corrected_.tif';
