clear; close all;

% === Paths ===
new_film_file = '/home/oem/eliza/data/xyz_lab_rgb/film/yoda_halogen_fuji_exp0.mat';  % NEW FILM
new_film_file2 = '/home/oem/eliza/data/xyz_lab_rgb/film/yoda_led_kodak_exp0.mat';  % NEW FILM

painting_before_file = '/home/oem/eliza/data/xyz_lab_rgb/hyspex/yoda_reflectance_before_xyz.mat';
cluster_metadata_file = '/home/oem/eliza/data/xyz_lab_rgb/clustered/yoda_spectral_cluster_metadata.mat';
painting_after_file = '/home/oem/eliza/data/xyz_lab_rgb/clustered/yoda_painting_after_cluster_rgb.mat';

% Load cluster metadata
load(cluster_metadata_file, 'cluster_idx', 'valid_mask', 'nColors');

% Load new film Lab/XYZ/RGB
film_data = load(new_film_file);
painting_before = load(painting_before_file);
Lab_film = reshape(film_data.Lab_img, [], 3);
XYZ_film = reshape(film_data.XYZ_img, [], 3);
RGB_film = reshape(film_data.RGB_img, [], 3);

% Apply valid mask
lab_film_valid = Lab_film(valid_mask(:), :);
xyz_film_valid = XYZ_film(valid_mask(:), :);
rgb_film_valid = RGB_film(valid_mask(:), :);

%% === Extract new film cluster means ===
lab_film_clustered = zeros(nColors, 3);
xyz_film_clustered = zeros(nColors, 3);
rgb_film_clustered = zeros(nColors, 3);

for k = 1:nColors
    mask_k = cluster_idx == k;
    lab_film_clustered(k,:) = mean(lab_film_valid(mask_k,:), 1, 'omitnan');
    xyz_film_clustered(k,:) = mean(xyz_film_valid(mask_k,:), 1, 'omitnan');
    rgb_film_clustered(k,:) = mean(rgb_film_valid(mask_k,:), 1, 'omitnan');
end

%% === Save the extracted film cluster data ===
[~, film_base_name, ~] = fileparts(new_film_file);
save_root = '/home/oem/eliza/data/xyz_lab_rgb/clustered/filtered';
save_dir = fullfile(save_root, film_base_name);

if ~exist(save_dir, 'dir')
    mkdir(save_dir);
end

% save(fullfile(save_dir, [film_base_name '_cluster.mat']), ...
%     'lab_film_clustered', 'xyz_film_clustered', 'rgb_film_clustered');

fprintf('Saved clustered film data to: %s\n', save_dir);

%%
%% === Visualize Clustered RGB Palette (Film) ===
srgb_film_clustered = xyz2rgb(xyz_film_clustered ./ 100, ...
    'ColorSpace', 'srgb', 'WhitePoint', 'd50');

patchSize = 60;
grid_w = 10;
grid_h = ceil(nColors / grid_w);
palette_img = ones(grid_h * patchSize, grid_w * patchSize, 3);



% === Diagonal Comparison with Painting (After) ===
painting_data = load(painting_after_file);
srgb_after = xyz2rgb(painting_data.xyz_after_clustered ./ 100, ...
    'ColorSpace', 'srgb', 'WhitePoint', 'd50');

if size(srgb_after,1) ~= size(srgb_film_clustered,1)
    error('Cluster count mismatch: painting (%d) vs film (%d)', ...
        size(srgb_after,1), size(srgb_film_clustered,1));
end

img_diag = ones(grid_h * patchSize, grid_w * patchSize, 3);
for k = 1:nColors
    row = floor((k - 1) / grid_w);
    col = mod((k - 1), grid_w);
    r_idx = (row * patchSize + 1):((row + 1) * patchSize);
    c_idx = (col * patchSize + 1):((col + 1) * patchSize);

    patch_painting = repmat(reshape(srgb_after(k,:), 1, 1, 3), patchSize, patchSize, 1);
    patch_film     = repmat(reshape(srgb_film_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);

    [X, Y] = meshgrid(1:patchSize, 1:patchSize);
    diag_mask = Y > X;

    patch = patch_painting;
    for c = 1:3
        temp = patch(:,:,c);
        temp2 = patch_film(:,:,c);
        temp(diag_mask) = temp2(diag_mask);
        patch(:,:,c) = temp;
    end

    img_diag(r_idx, c_idx, :) = patch;
end

figure('Position', [100 100 1200 1100]);
tiledlayout(1,1, 'Padding','compact', 'TileSpacing','compact');
nexttile;
imshow(img_diag);
title('Cluster Palette Comparison: After Painting (top right) vs Film (bottom left)', ...
    'FontSize', 20, 'FontWeight', 'bold');

% exportgraphics(gcf, fullfile(save_dir, [film_base_name '_comparison_after_vs_film.png']), 'Resolution', 300);


%%
%% === Cluster Painting Before Data and Compare with Film ===
Lab_before  = reshape(painting_before.Lab_img, [], 3);
XYZ_before  = reshape(painting_before.XYZ_img, [], 3);
RGB_before  = reshape(painting_before.RGB_img, [], 3);

lab_before_valid = Lab_before(valid_mask(:), :);
xyz_before_valid = XYZ_before(valid_mask(:), :);
rgb_before_valid = RGB_before(valid_mask(:), :);

% Compute cluster means for Painting Before
lab_before_clustered = zeros(nColors, 3);
xyz_before_clustered = zeros(nColors, 3);
rgb_before_clustered = zeros(nColors, 3);

for k = 1:nColors
    mask_k = cluster_idx == k;
    lab_before_clustered(k,:) = mean(lab_before_valid(mask_k,:), 1, 'omitnan');
    xyz_before_clustered(k,:) = mean(xyz_before_valid(mask_k,:), 1, 'omitnan');
    rgb_before_clustered(k,:) = mean(rgb_before_valid(mask_k,:), 1, 'omitnan');
end

% Convert to sRGB
srgb_before_clustered = xyz2rgb(xyz_before_clustered ./ 100, 'ColorSpace', 'srgb', 'WhitePoint', 'd50');

% === Diagonal Split Patch Plot (Painting Before vs Film) ===
img_diag = ones(grid_h * patchSize, grid_w * patchSize, 3);
for k = 1:nColors
    row = floor((k - 1) / grid_w);
    col = mod((k - 1), grid_w);
    r_idx = (row * patchSize + 1):((row + 1) * patchSize);
    c_idx = (col * patchSize + 1):((col + 1) * patchSize);

    patch_before = repmat(reshape(srgb_before_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);
    patch_film   = repmat(reshape(srgb_film_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);

    [X, Y] = meshgrid(1:patchSize, 1:patchSize);
    diag_mask = Y > X;

    patch = patch_before;
    for c = 1:3
        temp = patch(:,:,c);
        temp2 = patch_film(:,:,c);
        temp(diag_mask) = temp2(diag_mask);
        patch(:,:,c) = temp;
    end

    img_diag(r_idx, c_idx, :) = patch;
end

figure('Position', [100 100 1200 1100]);
tiledlayout(1,1, 'Padding','compact', 'TileSpacing','compact');
nexttile;
imshow(img_diag);
title('Cluster Palette Comparison: Painting Before (top right) vs Film (bottom left)', ...
    'FontSize', 20, 'FontWeight', 'bold');

% exportgraphics(gcf, fullfile(save_dir, [film_base_name '_comparison_before_vs_film.png']), 'Resolution', 300);


%% === Diagonal Split Patch Plot (Before vs After) ===
img_diag = ones(grid_h * patchSize, grid_w * patchSize, 3);
for k = 1:nColors
    row = floor((k - 1) / grid_w);
    col = mod((k - 1), grid_w);
    r_idx = (row * patchSize + 1):((row + 1) * patchSize);
    c_idx = (col * patchSize + 1):((col + 1) * patchSize);

    patch_before = repmat(reshape(srgb_before_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);
    patch_after  = repmat(reshape(srgb_after(k,:), 1, 1, 3), patchSize, patchSize, 1);

    [X, Y] = meshgrid(1:patchSize, 1:patchSize);
    diag_mask = Y > X;

    patch = patch_before;
    for c = 1:3
        temp = patch(:,:,c);
        temp2 = patch_after(:,:,c);
        temp(diag_mask) = temp2(diag_mask);
        patch(:,:,c) = temp;
    end

    img_diag(r_idx, c_idx, :) = patch;
end

figure('Position', [100 100 1200 1100]);
tiledlayout(1,1, 'Padding','compact', 'TileSpacing','compact');
nexttile;
imshow(img_diag);
title('Cluster Palette Comparison: Painting Before (top right) vs After (bottom left)', ...
    'FontSize', 20, 'FontWeight', 'bold');

% exportgraphics(gcf, fullfile(save_dir, [film_base_name '_palette_before_vs_after.png']), 'Resolution', 300);

%%
%% === Compute Delta E₀₀ Between Painting Before and After Clusters ===
DE_before_after = deltaE2000(lab_before_clustered, painting_data.lab_after_clustered);

% Convert to grid format
dE_grid_before_after = zeros(grid_h, grid_w);
for k = 1:nColors
    row = floor((k-1) / grid_w) + 1;
    col = mod((k-1), grid_w) + 1;
    dE_grid_before_after(row, col) = DE_before_after(k);
end

% Plot
figure('Position', [100 100 800 700]);
imagesc(dE_grid_before_after);
axis image off;
colormap(jet(256));
clim([0 10]);

cb = colorbar;
cb.Label.String = '\DeltaE_{00}';
cb.Label.FontSize = 16;
cb.Label.FontWeight = 'bold';
cb.FontSize = 16;
cb.FontWeight = 'bold';

title('\DeltaE_{00} Between Painting Before vs After', ...
    'FontSize', 18, 'FontWeight', 'bold');

% Overlay values
for k = 1:nColors
    row = floor((k-1)/grid_w) + 1;
    col = mod((k-1), grid_w) + 1;
    text(col, row, sprintf('%.1f', DE_before_after(k)), ...
        'HorizontalAlignment','center', 'Color','w', ...
        'FontSize', 12, 'FontWeight', 'bold');
end

% % Save
% exportgraphics(gcf, ...
%     fullfile(save_dir, [film_base_name '_palette_deltaE_before_vs_after.png']), ...
%     'Resolution', 300);


%% === Compare Clusters of Film 1 vs Film 2 ===
film_data2 = load(new_film_file2);
Lab_film2 = reshape(film_data2.Lab_img, [], 3);
XYZ_film2 = reshape(film_data2.XYZ_img, [], 3);

lab_film2_valid = Lab_film2(valid_mask(:), :);
xyz_film2_valid = XYZ_film2(valid_mask(:), :);

lab_film2_clustered = zeros(nColors, 3);
xyz_film2_clustered = zeros(nColors, 3);

for k = 1:nColors
    mask_k = cluster_idx == k;
    lab_film2_clustered(k,:) = mean(lab_film2_valid(mask_k,:), 1, 'omitnan');
    xyz_film2_clustered(k,:) = mean(xyz_film2_valid(mask_k,:), 1, 'omitnan');
end

srgb_film2_clustered = xyz2rgb(xyz_film2_clustered ./ 100, ...
    'ColorSpace', 'srgb', 'WhitePoint', 'd50');

% Diagonal visualisation: Film 1 (top right) vs Film 2 (bottom left)
img_diag = ones(grid_h * patchSize, grid_w * patchSize, 3);
for k = 1:nColors
    row = floor((k - 1) / grid_w);
    col = mod((k - 1), grid_w);
    r_idx = (row * patchSize + 1):((row + 1) * patchSize);
    c_idx = (col * patchSize + 1):((col + 1) * patchSize);

    patch1 = repmat(reshape(srgb_film_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);
    patch2 = repmat(reshape(srgb_film2_clustered(k,:), 1, 1, 3), patchSize, patchSize, 1);

    [X, Y] = meshgrid(1:patchSize, 1:patchSize);
    diag_mask = Y > X;

    patch = patch1;
    for c = 1:3
        temp = patch(:,:,c);
        temp2 = patch2(:,:,c);
        temp(diag_mask) = temp2(diag_mask);
        patch(:,:,c) = temp;
    end

    img_diag(r_idx, c_idx, :) = patch;
end

figure('Position', [100 100 1200 1100]);
tiledlayout(1,1, 'Padding','compact', 'TileSpacing','compact');
nexttile;
imshow(img_diag);
title('Cluster Comparison: Fuji Halogen (top right) vs Kodak LED (bottom left)', ...
    'FontSize', 20, 'FontWeight', 'bold');

exportgraphics(gcf, fullfile(save_dir, [film_base_name '_comparison_film1_vs_film2.png']), 'Resolution', 300);
